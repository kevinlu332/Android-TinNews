...
public class NewsRepository {
    ...
    public LiveData<NewsResponse> searchNews(String query){
        ...
        newsApi.getEverything(query, 40).enqueue(new Callback<NewsResponse>() {
            @Override
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                if(response.isSuccessful()) everyThingLiveData.setValue(response.body());
                else everyThingLiveData.setValue(null);
            }
    ...
    //use an AsyncTask to dispatch the query work to a background thread.
    ...
    //add an API to do the action of favorite an Article, in which it execute the background async task
